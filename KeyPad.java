...
Клавиатура для ввода букв. Содержит имя игрока, не длиннее 15 символов.
Есть кнопка открытия случайной буквы из слова. Буквы на клавиатуре только заглавные.

Keypad в методе actionPerformed передает действие в WodGuessWindow через интерфейс KayPadClient.

Кнопки меняют вид в зависимости от состояния кнопки по средствам смены Border.

Стиль кнопки для открытия случайной буквы прописан прямо в конструкторе, поскольку он используется только один
раз, только в этом месте и нигде больше. Создание отдельного стиля в данном случаене не оправданно и приведет к загромождению кода.

буква Е воспринимается также как и буква Ё.
*/
...
public class KeyPad extends JPanel implements ActionListener{
	...
	private final static int NLETTERS = 32;//Ё не отображается
	...
	KeyPad(KeyPadClient kpc){this("Игрок", kpc);}
	KeyPad(String name, KeyPadClient kpc){
		...
		for(int i=NLETTERS-6;i<NLETTERS-3;i+=2){//твёрдый знак и мягкий знак.
			lables[i] = (char)((int)'А'+i);
			buttons[i]=new JButton(Character.toString(lables[i]));
			buttons[i].addActionListener(this);		
		}//fot i
	...
	public void actionPerformed(ActionEvent e){
		if(active){
			if(e.getSource()==randomLetter)
				kpc.openRandomLetter();
			else
				kpc.checkLetter( ((JButton)e.getSource()).getText().charAt(0) );
		}//if(active)
	}
	...
	public void setActive(boolean bul){
		if(bul)
			{active=true;pad.setBorder(borderActive);}
		else 
			{active=false;pad.setBorder(borderNotActive);}	
	}
